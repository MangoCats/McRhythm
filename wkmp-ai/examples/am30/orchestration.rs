...
/// * `source_artists` - Artist name variants from ID3 tags/path (for Stage 4 gating)
/// * `source_albums` - Album name variants from ID3 tags/path (for Stage 4 gating)
///
/// # Returns
...
pub(crate) fn test_single_edition(
    ...
    if !stage2_results.over_segmented_candidates.is_empty() {
        ...
        if let Some(result) = run_stage3_single_edition(
            ...
            // Stage 3 divergence check: artist_similarity >= album_similarity × STAGE4_MIN_ARTIST_ALBUM_RATIO
            // Rejects wrong-artist matches where album name coincidentally matches (e.g., "Anthology", "Atlas")
            let divergence = check_divergence(&edition.artist, &edition.album, source_artists, source_albums);

            if !divergence.passes {
                info!(
                    "[A{}] Stage 3 BLOCKED by divergence check: artist={:.1}% < album={:.1}% × ratio = {:.1}% (edition: {} - {})",
                    album_idx + 1,
                    divergence.artist_similarity * 100.0,
                    divergence.album_similarity * 100.0,
    ...
    // Run 29: Adds artist gating to prevent false positives
    if !rms_profile.is_empty() {
        log_messages.push("    Stage 4: Guided quiet spot detection (with artist gating)...".to_string());
