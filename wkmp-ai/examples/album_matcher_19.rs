...
///
/// Run 19 Changes: Parallel MB lookup + silence detection after decode
/// Run 18 Changes:
...
async fn process_single_album(
    ...
    // === RUN 19: PARALLEL MB LOOKUP + SILENCE DETECTION ===
    // Start both operations concurrently after decode completes
    info!("[A{}]   Starting parallel: MusicBrainz lookup + silence detection...", album_idx + 1);
    ...
    // MusicBrainz lookup (async, I/O-bound)
    ...
    let (silence_result, mb_result) = tokio::join!(silence_task, mb_task);

    // Extract silence cache (panic if spawn_blocking failed)
    let silence_cache = silence_result.expect("Silence detection task panicked");
