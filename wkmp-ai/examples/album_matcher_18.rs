/// Comprehensive Album Matcher with Edition-by-Edition Processing (Run 18)
///
/// Run 18 Changes:
/// - ALBUM-PREFIXED LOGGING: All album-specific log messages prefixed with [A{num}]
///   for easier filtering and debugging when processing multiple albums in parallel
/// - SINGLE-PASS SILENCE DETECTION: Instead of scanning 180 times (once per parameter
///   combination), scan ONCE at the most permissive threshold and record actual dB
///   levels. Then filter the master gap list for each parameter combo. Expected 10-50x
///   speedup on silence cache pre-computation phase.
///
/// Run 17 Changes:
/// - MULTI-ALBUM PARALLELISM: Process N albums concurrently with shared rate limiter
/// - Decode and MB lookup now run in parallel WITHIN each album (pipeline parallelism)
/// - Multiple albums process concurrently (buffer_unordered with MAX_CONCURRENT_ALBUMS)
/// - Shared RateLimiter ensures MB API rate limits are respected across all parallel albums
/// - Expected 2-3x overall speedup: While Album 1 decodes, Album 2 does MB lookups
/// - Memory-bounded: MAX_CONCURRENT_ALBUMS=3 limits peak RAM to ~500MB (3 × 170MB PCM)
/// - FINAL RESULT now includes winning Artist and Album names for cross-referencing
///
/// Run 16 Changes:
...
// Memory impact: ~170MB PCM per album, so 3 albums ≈ 500MB peak RAM
const MAX_CONCURRENT_ALBUMS: usize = 3;
...
async fn process_single_album(
    ...
    // Start MB lookup concurrently (network-bound, uses shared rate limiter)
    // Note: We need to estimate file duration for MB search - use a placeholder initially
    // The actual filtering will happen after decode completes
    info!("[A{}]   Starting parallel decode + MusicBrainz lookup...", album_idx + 1);

    // Decode MP3 (wait for completion)
    info!("[A{}]   Decoding...", album_idx + 1);
    let (samples, sample_rate) = match decode_handle.await {
        Ok(Ok((s, sr))) => {
    ...
    // Now do MusicBrainz lookup with actual file duration
    ...
    let editions = match comprehensive_musicbrainz_search(
        ...
        Ok(releases) => {
            ...
            let mut editions = group_into_editions(releases, album_idx);
            ...
            info!("[A{}]   Sorted editions by likelihood (file: {:.0}s, ~{} tracks)",
                album_idx + 1, file_duration_secs, initial_durations.len());
            ...
            info!("[A{}] Found {} unique editions to test", album_idx + 1, editions.len());
    ...
    // Single-pass silence detection: scan once at -30dB, filter for 180 param combinations
    info!("[A{}]   Single-pass silence detection (1 scan → {} param combinations, {} threads)...",
        album_idx + 1, threshold_values.len() * min_duration_values.len(), rayon::current_num_threads());
    ...
    info!("[A{}]   Silence cache ready ({} entries)", album_idx + 1, silence_cache.len());
    ...
    info!("[A{}]   === EDITION-BY-EDITION PROCESSING (Run 18 - STAGGERED FEED + EARLY EXIT) ===", album_idx + 1);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("=== Comprehensive Album Matcher (Run 18) ===");
    ...
    info!("  - RUN 18: Album-prefixed logging [A{{num}}] for parallel debugging");
    ...
    info!("=== Run 18: Multi-Album Parallel Processing ===");
