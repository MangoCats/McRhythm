...
/// Comprehensive Album Matcher with Edition-by-Edition Processing (Run 23 - Single-Track Detection)
...
fn find_best_edition_result_with_artist_check<'a>(
    ...
    info!(
        "[A{}]   🔍 Artist mismatch detected ('{}'→'{}', sim={:.1}%), evaluating runner-ups...",
        album_idx + 1,
        source_artist,
        winner_artist,
        winner_sim * 100.0
        ...
        .take(num_candidates - 1)
    {
        ...
        debug!(
            "[A{}]         Artist: min={} delta={} ratio={} → better={}",
            album_idx + 1,
            meets_min_similarity,
            meets_delta,
            meets_ratio,
            artist_significantly_better
        );
        debug!(
            "[A{}]         Time: delta={:.3} (pct={:.3} + err={:.3}) → acceptable={}",
            album_idx + 1,
            time_fit_delta,
            pct_ratio_term,
            error_ratio_term,
        ...
        if artist_significantly_better && time_fit_acceptable {
            info!(
                "[A{}]   🎯 Artist fallback: Promoting runner-up #{} over winner",
                album_idx + 1,
                rank + 1
            );
            info!(
                "[A{}]       Winner: '{}' (sim={:.1}%, pct={:.1}%, err={:.2}s)",
                album_idx + 1,
                winner_artist,
                winner_sim * 100.0,
                winner_pct,
            ...
            return (
                Some(runner),
                true,
                Some((winner_idx, winner_pct, winner_artist.clone(), winner_sim)),
            );
        }
    }
    ...
    warn!(
        "[A{}]   ⚠️ No suitable artist-matched runner-up found",
        album_idx + 1
    );
    warn!(
        "[A{}]       Using original winner: '{}' (sim={:.1}%, pct={:.1}%)",
        album_idx + 1,
        winner_artist,
        winner_sim * 100.0,
        winner_pct
