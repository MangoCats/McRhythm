...
// =============================================================================
// Minimum Levenshtein ratio (0.0-1.0) required for artist name similarity.
// Candidates with artist ratio below this are filtered out before NDR ranking.
// 0.5 = requires at least 50% character similarity between source and candidate artist.
// Example: "Metallica" vs "The Ride" = 0.11 (filtered), "Bjork" vs "Björk" = 0.80 (kept)
const MIN_ARTIST_LEVENSHTEIN_RATIO: f64 = 0.5;

// Minimum Levenshtein ratio (0.0-1.0) required for album name similarity.
// Candidates with album ratio below this are filtered out before NDR ranking.
// 0.5 = requires at least 50% character similarity between source and candidate album.
// Example: "Ride the Lightning" vs "The Ride" = 0.31 (filtered), "Gaucho" vs "Gaucho" = 1.0 (kept)
const MIN_ALBUM_LEVENSHTEIN_RATIO: f64 = 0.5;
...
///
/// Run 24: Applies a two-stage filter:
/// 1. Pre-NDR ratio filter: Requires artist ratio >= MIN_ARTIST_LEVENSHTEIN_RATIO AND
///    album ratio >= MIN_ALBUM_LEVENSHTEIN_RATIO. This catches obvious mismatches early.
...
fn calculate_ndr_and_filter<'a>(
    ...
    // === Run 24: Stage 1 - Pre-NDR Levenshtein Ratio Filter ===
    // Filter out releases where artist OR album ratio is below threshold.
    // This catches obvious mismatches (e.g., "Metallica" vs "The Ride") before NDR ranking.
    let ratio_filtered: Vec<&MBRelease> = releases
        ...
        .filter(|release| {
            ...
            artist_ratio >= MIN_ARTIST_LEVENSHTEIN_RATIO && album_ratio >= MIN_ALBUM_LEVENSHTEIN_RATIO
        })
        .collect();

    let ratio_filtered_count = initial_count - ratio_filtered.len();
    if ratio_filtered_count > 0 {
        info!("[A{}]   Run 24 ratio filter: skipping {} releases (artist<{:.0}% OR album<{:.0}%), {} remain",
              album_idx + 1, ratio_filtered_count,
              MIN_ARTIST_LEVENSHTEIN_RATIO * 100.0, MIN_ALBUM_LEVENSHTEIN_RATIO * 100.0,
              ratio_filtered.len());
    }
