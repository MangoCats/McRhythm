...
pub(crate) async fn process_single_album(
    ...
    for (edition_idx, edition) in editions.iter().enumerate() {
        ...
        // Run 29: Clone artist/album variants for Stage 4 artist gating
        let artist_variants_clone = artist_variants.clone();
        let album_variants_clone = album_variants.clone();
        ...
        rayon::spawn(move || {
            let edition_result = panic::catch_unwind(panic::AssertUnwindSafe(|| {
                test_single_edition(
                    ...
                    parallel_start_time,
                    album_idx,
                    &artist_variants_clone,
                    &album_variants_clone,
                )
            }));
