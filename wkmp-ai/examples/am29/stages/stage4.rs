...
pub(crate) fn run_stage4_single_edition(
    ...
    // === LAYER 1: Pre-filter Gate ===
    // Compute artist similarity using Jaro-Winkler - skip Stage 4 entirely if too low
    // Run 29b: Changed from Levenshtein ratio to Jaro-Winkler because Levenshtein gave
    // false 66.7% for "Fluke" vs "ゆいかおり" (same length, different charset)
    // while Jaro-Winkler correctly gives ~0%
