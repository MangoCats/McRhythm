...
## Overview
McRhythm is a music player that selects passages to play based on user preferences for style at various times of day.
...
- Cross references passages to the AcousticBrainz database when possible, identifying musical character of each passage
  - In absence of AcousticBrainz data will use AcousticBrainz algorithms locally to generate musical character values for the passage
...
## Three Versions
- Full version includes player, auto song selector and all database building / maintenance and preference editing functions
- Lite version includes player, auto song selector and preference editing functions, uses a static database built by the full version
- Minimal version includes only the player and auto song selector, using database and preferences built by a Full or lite version
...
- Passages have a lead-in point identified at some time at or after the start time, at or before the lead-out point. The lead-in time is the difference between the lead-in point and the start time.
...
- When fade-in time is greater than 0, the recorded audio level in the file is faded in, starting at 0 at the pasage start time and ending at the full current volume level at the fade-in point
...
- passages are selected to be played based on multiple criteria:
  ...
  - A user configured preference for the kind of passages to be played at different times of day
