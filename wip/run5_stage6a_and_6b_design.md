# Run 6 - Stage 6A & 6B Implementation Design

**Document:** Design Specification for Boundary Refinement and Regional Re-scanning
**Date:** 2025-11-19
**Status:** Implementation Complete
**Target:** Run 6 Implementation

---

## Executive Summary

Run 6 extends Run 5's 6-phase pipeline to an **8-phase pipeline** with two advanced boundary optimization stages:

### Background: Run 5 Foundation

Run 5 established the comprehensive 6-phase matching pipeline:
- **Phase 0:** ID3 Tag Extraction & Reconciliation
- **Stages 1-4:** Initial detection, parameter optimization, segment assembly, quiet spot detection
- **Stage 5:** Expanded MusicBrainz candidate search

Run 5 achieved 88.9% Excellent match rate (16/18 albums) with 94.4% overall success rate (17/18 albums).

### New in Run 6: Advanced Boundary Optimization

Run 6 adds **two complementary refinement stages** that activate when track count is correct but match quality < 100%:

### Stage 6A: Local Boundary Refinement

Optimizes **individual track boundaries** when adjacent tracks show opposite-sign errors:
- Detects boundary misplacement patterns (one track too long, next too short)
- Searches for better quiet spots in window around current boundary
- Uses RMS amplitude analysis for precise boundary placement
- **Target albums:** The Police - Zenyatta Mondatta (81.8%), Toto IV (80.0%)

### Stage 6B: Regional Parameter Re-scan

Re-segments **problem regions** (≥2 consecutive mismatched tracks) with alternate parameters:
- Identifies heterogeneous albums requiring different parameters for different sections
- Tests 6 alternate parameter combinations on each problem region
- Preserves well-matched regions unchanged
- **Target albums:** The Rolling Stones - Let It Bleed (66.7%, first 4 tracks), Toto IV track 4

**Key Innovation:** Two-tiered optimization: Stage 6A for local fixes, Stage 6B for regional re-segmentation, enabling handling of heterogeneous albums and boundary misplacements.

---

## High-Level Process Flow

### 8-Phase Matching Pipeline (Run 6)
...
Stage 6A: Local Boundary Refinement ← NEW (Run 6)
    ↓ (if < 100% match AND track count correct)
Stage 6B: Regional Parameter Re-scan ← NEW (Run 6)
    ↓
Final Result
```

### Stage 6A/6B Trigger Conditions

Both Stage 6A and Stage 6B execute when **ALL** conditions met:
1. **best_percentage < 100.0** (not yet perfect match)
2. **Track count correct** (detected_count == expected_count)
3. **Stage 6A runs first**, then Stage 6B (sequential refinement)
