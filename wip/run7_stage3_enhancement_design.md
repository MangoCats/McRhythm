# Run 7 - Stage 3 Comprehensive Assembly Enhancement Design

**Document:** Design Specification for Comprehensive Segment Assembly
**Date:** 2025-11-20
**Status:** Implementation Complete
**Target:** Run 7 Implementation

---

## Executive Summary

Run 7 enhances Run 6's **Stage 3: Segment Assembly** from testing a **single** over-segmented candidate to testing **ALL** over-segmented candidates from Stages 1 and 2.

### Background: Run 6 Performance

Run 6 established an 8-phase pipeline with Stages 6A and 6B for boundary refinement:
- **Phase 0:** ID3 Tag Extraction & Reconciliation
- **Stages 1-2:** Initial detection, parameter optimization (180 combinations)
- **Stage 3:** Segment assembly (single candidate only)
- **Stages 4-5:** Quiet spot detection, expanded MusicBrainz search
- **Stages 6A-6B:** Local boundary refinement, regional parameter re-scan

**Run 6 Limitation Identified:**
- **The Police - Zenyatta Mondatta** remained at 81.8% match despite Stage 6A/6B refinement
- Tracks 2-3 had severe boundary misplacement (±195s errors)
- Stage 5 collected **132 unique segmentations**, but Stage 3 only assembled **one** of them
- **Hypothesis:** One of the other 131 segmentations, when assembled, might yield perfect boundaries

### New in Run 7: Comprehensive Assembly Search

Run 7 enhances **Stage 3** to test assembly on **ALL** over-segmented candidates:

**Previous Stage 3 (Run 6):**
- Tested assembly on **current best** segmentation only
- Typically 1 assembly tested per album

**Enhanced Stage 3 (Run 7):**
- Tests assembly on **ALL** over-segmented segmentations from Stages 1 & 2
- Potentially 50-150+ assemblies tested per album
- **22× more assembly candidates** for Zenyatta Mondatta (132 vs. 6 from Stage 6B)
- Early exit on 100% match

**Key Innovation:** Dramatically expand assembly search space by leveraging **all** over-segmented candidates generated during parameter optimization, not just the current best. This transforms Stage 3 from a single-shot refinement into a comprehensive boundary optimization search.

### Expected Impact

**For The Police - Zenyatta Mondatta specifically:**
- Run 6: 81.8% match (tracks 2-3 boundary severely misplaced)
- 132 segmentations available from Stage 2
- Likely many over-segmented that region differently
- **Expected Run 7: 100% match** via optimal assembly of over-segmented candidate

---

## High-Level Process Flow

### 8-Phase Matching Pipeline (Run 7)
...
Stage 2: Parameter Optimization (180-grid search)
    ↓ (stores ALL segmentations)
Stage 3: COMPREHENSIVE Segment Assembly ← ENHANCED (Run 7)
    ↓ (tests assembly on ALL over-segmented candidates from Stages 1 & 2)
    ↓ (if < 100% match)
...
Stage 5: Expanded MusicBrainz Search
    ↓ (if < 100% match AND track count correct)
Stage 6A: Local Boundary Refinement (Run 6)
    ↓ (if < 100% match AND track count correct)
Stage 6B: Regional Parameter Re-scan (Run 6)
    ↓
Final Result
```

### Stage 3 Enhancement Details

**Previous behavior (Run 6):**
1. Check if current best is over-segmented
2. If yes, try assembly on current best
3. Update best if improved

**New behavior (Run 7):**
1. Collect ALL over-segmented candidates from Stages 1 & 2
2. Try assembly on EACH over-segmented candidate
3. Track best assembly result across all candidates
4. Early exit on 100% match

**Trigger Condition:** best_percentage < 100.0

**Performance:** ~1 second for 100+ assemblies (DP algorithm is fast)
...
**Purpose:** Find optimal track boundaries by testing dynamic programming assembly on ALL over-segmented candidates from Stages 1 and 2, not just the current best.

**Trigger Conditions:**
- best_percentage < 100.0
- At least one over-segmented candidate exists (track count > expected)

### Problem Addressed

**Limited Assembly Search in Run 6:**

In Run 6, Stage 3 only tested assembly on the **current best** segmentation:
- If current best had 13 tracks (expected: 11), Stage 3 would try assembling those 13 tracks
- **Ignored** all other over-segmented results from Stage 2
- Example: Zenyatta Mondatta had **132 unique segmentations**, but only **1** was assembled

**The Police - Zenyatta Mondatta Case Study (Run 6):**
- **Stage 2** tested 180 parameter combinations, generating 132 unique segmentations
- **Stage 3** assembled only the current best (13 tracks → 11 tracks, 81.8% match)
- **Problem:** Tracks 2-3 boundary severely misplaced (±195s errors)
- **Hypothesis:** One of the other 131 segmentations likely over-segmented tracks 2-3 differently
  - Example: Might have 3 segments in that region totaling ~417s
  - Assembling optimally: Could yield (200s + 217s) = perfect match
- **Result:** Stuck at 81.8% despite Stage 6A/6B refinement

### Enhancement Design

**Run 7 Approach: Comprehensive Assembly Search**

Instead of assembling only the current best, try assembly on **ALL** over-segmented candidates:

#### Algorithm

```
1. Collect over-segmented candidates:
   - Stage 1 result (if over-segmented)
   - ALL Stage 2 results (if over-segmented)

2. For each over-segmented candidate:
   a. Apply dynamic programming assembly
   b. Evaluate match quality
   c. Update best if improved
   d. Early exit if 100% match found

3. Report:
   - Number of assemblies tested
   - Number that improved over previous best
   - Best assembly source and percentage
```

#### Dynamic Programming Assembly

The `assemble_segments_dp()` function uses dynamic programming to optimally merge over-segmented tracks:

- **Input:** Detected segments (e.g., 13 tracks), expected track count (e.g., 11)
- **Output:** Optimal grouping that minimizes total duration error
- **Complexity:** O(n²k) where n=segments, k=expected tracks
- **Performance:** ~1ms for typical album (13 segments → 11 tracks)

**Example:**
```
Detected: [s1=180s, s2=20s, s3=180s, s4=37s, ...]  (13 segments)
Expected: [200s, 217s, ...]  (11 tracks)
...
| Metric | Run 6 | Run 7 | Improvement |
|--------|-------|-------|-------------|
| **Assemblies tested** | 1 | 50-150+ | 50-150× |
| **Search space** | Current best only | ALL over-segmented | Comprehensive |
| **Zenyatta Mondatta** | 1 assembly | 132 assemblies | **132×** |

**Expected Album-Specific Results:**

| Album | Run 6 | Stage 3 Candidates | Expected Run 7 |
|-------|-------|-------------------|----------------|
| The Police - Zenyatta Mondatta | 81.8% | 132 assemblies | **100%** |
| Toto IV | 80.0% | ~120 assemblies | **90-100%** |
| The Rolling Stones - Let It Bleed | 66.7% | ~100 assemblies | **88%+** |

**Rationale:** Many parameter combinations over-segment albums differently. By testing ALL assemblies, dramatically increase probability of finding optimal track boundaries.

### Implementation Details

**Code Changes (album_matcher.rs lines 2202-2277):**

```rust
// Collect all over-segmented candidates
let mut over_segmented_candidates = Vec::new();

// Stage 1 result
if stage1_durations.len() > expected_durations.len() {
    over_segmented_candidates.push((
        stage1_durations.clone(),
        "Initial".to_string(),
        None,
        None
    ));
...
// Test assembly on each candidate
for (candidate_durations, source, thresh, min_dur) in over_segmented_candidates {
    if let Some(assembled) = assemble_segments_dp(&candidate_durations, &expected_durations) {
        let (_, _, percentage) = analyze_track_matching(&assembled, &expected_durations, tolerance);

        if percentage > best_percentage {
            best_percentage = percentage;
            best_durations = assembled;
            // ... update best result
...
**Performance Characteristics:**
- **Per-assembly time:** ~1ms (DP algorithm is fast)
- **Total Stage 3 time:** ~100-150ms for 100-150 assemblies
- **Negligible compared to MusicBrainz rate limiting** (2000ms per request)

### Example Output

```
STAGE 3: Comprehensive segment assembly across 132 over-segmented candidates...
  New best: 90.9% via assembly of Parameter Optimization (-52dB, 0.3s) (15 segments → 11 tracks)
  New best: 100.0% via assembly of Parameter Optimization (-54dB, 0.5s) (13 segments → 11 tracks)
  Tested 132 assemblies, 2 improved over best
  Best assembly: 100.0% from Parameter Optimization (-54dB, 0.5s)
```

---

## Stage 6A: Local Boundary Refinement

**Purpose:** Optimize individual track boundaries when track count is correct but boundary positions are suboptimal.

**Trigger Conditions:**
- best_percentage < 100.0
- detected_track_count == expected_track_count
- Runs BEFORE Stage 6B

### Problem Addressed

**Boundary Misplacement Pattern:** Adjacent tracks with opposite-sign errors indicate boundary in wrong position:
- Track N: Detected too long (error = +X seconds)
- Track N+1: Detected too short (error = -X seconds)
- **Root cause:** Boundary between tracks N and N+1 is misplaced

**Example from Run 5 (The Police - Zenyatta Mondatta):**
```
Track 2:    0.5s vs  200s  error=+199.5s  (WAY too short - boundary too early)
Track 3:  412.5s vs  217s  error=+195.5s  (WAY too long - boundary too late)
```

### Expected Improvements

**The Police - Zenyatta Mondatta:**
- Current: 81.8% (9/11 tracks matched)
- Tracks 2-3: Severe opposite-error pair (±200s errors)
- **Expected:** 100% after boundary refinement

**Toto IV:**
- Current: 80.0% (8/10 tracks matched)
- Tracks 1-2: Opposite-error pair (track 1 too long, track 2 too short)
- **Expected:** 90%+ after boundary refinement

---

## Stage 6B: Regional Parameter Re-scan

**Purpose:** Re-segment problem regions (≥2 consecutive mismatched tracks) with alternate parameters to handle heterogeneous albums.

**Trigger Conditions:**
- best_percentage < 100.0
- detected_track_count == expected_track_count
- Runs AFTER Stage 6A (if 6A didn't achieve 100%)

### Problem Addressed

**Heterogeneous Albums:** Different album sections require different silence detection parameters:
- Example: Quiet classical passages vs. loud rock sections
- Example: Live album with varying crowd noise between tracks
- Example: Compilation with tracks from different mastering sessions

**Example from Run 5 (Let It Bleed):**
```
Tracks 1-4: Mismatched (66.7% overall)
Tracks 5-9: Well matched
```

### Alternate Parameter Sets

Six combinations covering the spectrum:

| Threshold | Duration | Use Case |
|-----------|----------|----------|
| -44 dB | 0.5s | Very lenient - loud/noisy inter-track passages |
| -48 dB | 0.3s | Lenient - moderately loud passages |
| -52 dB | 0.3s | Moderate - balanced |
| -56 dB | 0.2s | Moderate-strict - typical silence |
| -60 dB | 0.2s | Strict - quiet albums |
| -60 dB | 0.1s | Very strict - short silence gaps |

### Expected Improvements

**Let It Bleed:**
- Current: 66.7% (6/9 tracks matched)
- Tracks 1-4: Problem region (regional failure)
- **Expected:** 88%+ after re-scanning tracks 1-4 with alternate parameters

---

## Expected Run 6 Outcomes

### Target Albums for Improvement

| Album | Run 5 Result | Stage | Expected Run 6 | Improvement |
|-------|--------------|-------|----------------|-------------|
| The Police - Zenyatta Mondatta | 81.8% | 6A | 100% | +18.2% |
| Toto IV | 80.0% | 6A/6B | 90-100% | +10-20% |
| The Rolling Stones - Let It Bleed | 66.7% | 6B | 88%+ | +21%+ |

### Overall Run 6 Goals

**Run 5 baseline:** 88.9% Excellent rate (16/18), 94.4% success rate (17/18)

**Run 6 targets:**
- **Excellent rate:** 94%+ (17/18) - Adding 1-2 albums to Excellent tier
- **Perfect matches:** 55%+ (10/18) - Up from 44.4% in Run 5
- **Fair-tier albums:** 0-1 (down from 2 in Run 5)
...
**Status:** Implementation complete ✓
**Code location:** `wkmp-ai/examples/album_matcher.rs`
**Compilation:** Successful ✓
**Testing:** Ready for Run 6
